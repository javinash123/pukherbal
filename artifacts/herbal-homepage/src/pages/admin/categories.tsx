...
export default function AdminCategories() {
  ...
  const openEdit = (cat: Category) => {
    setForm({ name: cat.name, description: cat.description || "", imageUrl: cat.imageUrl || "", sortOrder: cat.sortOrder, active: cat.active, metaTitle: cat.metaTitle || "", metaDescription: cat.metaDescription || "", metaKeywords: cat.metaKeywords || "" });
    setEditId(cat.id);
    setShowForm(true);
    setError("");
    window.scrollTo(0, 0);
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
            ...
            <div className="flex items-center gap-3">
              <p className="text-sm text-gray-500">{filtered.length} categor{filtered.length !== 1 ? "ies" : "y"}</p>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="cat-active" className="text-sm text-gray-700">Active (visible on website)</label>
                  </div>
                </div>
                {/* SEO Section */}
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div>
                      ...
                      <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" value={form.metaTitle} onChange={e => setForm(f => ({ ...f, metaTitle: e.target.value }))} placeholder={`${form.name || "Category Name"} | Pukhraj Herbals`} />
                    </div>
                    <div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {pageItems.map((cat) => (
                      <tr key={cat.id} className="hover:bg-gray-50">
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-3">
                            {cat.imageUrl ? (
                              <img src={cat.imageUrl} alt={cat.name} className="w-10 h-10 object-cover rounded-lg border border-gray-200 flex-shrink-0" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                            ) : (
                    ...
                    {pageItems.length === 0 && (
                      <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">{search ? "No categories match your search." : "No categories yet. Click \"+ Add Category\" to create one."}</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
